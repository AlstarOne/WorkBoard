...
2. **It doesn't fire on a fixed cadence.** Letta re-sends its memory **every turn**; mem0 and claude-mem run a write **every session** (whether or not anything new is worth saving). WorkBoard writes **only when there's something to record** — not per turn, not per session.
